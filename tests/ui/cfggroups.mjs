...
const GROUPS=['gPay','gPeriod','gSched','gPrem','gOff','gLook','gData'];
...
let p=await boot();
console.log('\n━━ Settings opens to seven headings, not fifty controls ━━');
const secs=await p.$$eval('#cfg .cfgsec',es=>es.map(e=>({id:e.id,open:e.open,
  title:e.querySelector('.cfgt').textContent})));
ok('seven groups', secs.length===7, secs.map(s=>s.title).join(' | '));
...
ok('pay names the rate and the rule', /37\.78/.test(sums[0]) && /40 h a week/.test(sums[0]), sums[0]);
ok('period names its length and payday', /14 days/.test(sums[1]) && /13 days after/.test(sums[1]), sums[1]);
ok('schedule names hours, days and lunch',
   /2:00 PM–10:30 PM/.test(sums[2]) && /Sun Mon Tue Wed Thu/.test(sums[2]) && /30 min lunch/.test(sums[2]), sums[2]);
ok('premiums names the differential', /\$0\.15\/h/.test(sums[3]) && /6:00 PM/.test(sums[3]), sums[3]);
ok('time off counts what you have', /holidays/.test(sums[4]) && /1 vacation/.test(sums[4]), sums[4]);
ok('appearance names the theme and clock', /12-hour/.test(sums[5]) && /sound on/.test(sums[5]), sums[5]);
ok('your data counts the shifts', /2 shifts/.test(sums[6]), sums[6]);

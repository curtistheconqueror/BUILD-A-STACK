...
- **D·2 "Keyboard" tab** 🔄 IN SANDBOX — building in `Downloads\keyboardproto.html` (NOT yet promoted into the app). Standard PC keyboard done through **v5**: full-size 104-key layout; **single-click = focus** (dots on partner keys), **double-click = 🔒 hold** then single-click partners to reveal connections **one at a time**; "Show all" firehose toggle; **Stack-Map-style relationship lines**; ~55 shortcuts; **4 color-coded categories** (Editing=cyan, Move&select=green, Windows&tabs=purple, System=pink) with a **clickable legend filter** at top; key-press "thunk" + optional sound. Logi **K860 photo received** (ergonomic split-wave, dual ⌘/Win legends). **Next:** build the K860 as a 2nd selectable board + the **PC⇄Mac toggle** (lights up its dual legends), then promote the winner into the app as the 4th top-level tab.

...
> **PHASE E SHELL → FULLY BUILT + PERSISTENT.** All five Start Here stages are real (no shells). Each is a data-driven array + renderer (`PRETEST`/`STEPS`/`DIVES`/`TRICKS` + `GRAD_STAGES`), scoped under `#view-start .sh-*`, feeding one overall N/5 tracker. **Progress now persists** via `localStorage` (key `bas_starthere_v1`): every pick/step/card is saved and replayed on load (sequential step-unlock restores correctly), with a "↺ reset my progress" link in Graduation. NOTE for future edits: `SH_PANES`/`shDone`/`GRAD_STAGES` are declared at the TOP of the Start Here script (before the renderers) because the restore loops call `shMarkDone()` during initial execution — moving them back below the renderers reintroduces a TDZ crash on reload-with-saved-state. Remaining E polish is optional: **E·4 "Did You Know" popup engine** (reusable, deferred).
